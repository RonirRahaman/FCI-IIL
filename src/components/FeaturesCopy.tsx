...
const items = [
  ...
  { title: 'Nationwide Network', desc: 'Access off-market properties, opportunities, and partners.' },
]
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 overflow-hidden">
      ...
      >
        ...
        <p className="mt-3 text-black/80">Reliable funding, deep expertise, and a partnership mindset.</p>
      </motion.div>
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((f, i) => (
          <motion.div
            ...
            viewport={{ once: true, margin: "-100px" }}
            className="rounded-xl bg-black/5 border border-black/10 p-6 h-full hover:border-black/20 transition shadow-sm hover:shadow-[0_8px_30px_rgba(195,206,211,0.1)]"
          >
            <h3 className="text-xl font-semibold">{f.title}</h3>
            <p className="text-black/75 mt-2">{f.desc}</p>
          </motion.div>
        ))}
      </div>
    </section>
